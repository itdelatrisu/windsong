...
public class Options {
	...
	public static final File LOG_FILE = new File(CONFIG_DIR, ".potato.log");

	/** File for storing user options. */
	private static final File OPTIONS_FILE = new File(CONFIG_DIR, ".potato.cfg");
	...
	/** Repository address. */
	public static final URI REPOSITORY_URI = URI.create("https://github.com/itdelatrisu/potato");

	/** Issue reporting address. */
	public static final String ISSUES_URL = "https://github.com/itdelatrisu/potato/issues/new?title=%s&body=%s";
	...
	private static File getXDGBaseDir(String env, String fallback) {
		...
		if (OS.indexOf("nix") >= 0 || OS.indexOf("nux") >= 0 || OS.indexOf("aix") > 0) {
			...
			File dir = new File(rootPath, "potato");
			if (!dir.isDirectory() && !dir.mkdir())
				ErrorHandler.error(String.format("Failed to create configuration folder at '%s/potato'.", rootPath), null, false);
			return dir;
		} else
			return new File("./");
	}
	 ...
	 */
	public static PotatoMap getThemeMap() {
		...
		PotatoMap map = new PotatoMap(null);
	...
	public static void saveOptions() {
		try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(
			...
			writer.write("# potato configuration");
			writer.newLine();
